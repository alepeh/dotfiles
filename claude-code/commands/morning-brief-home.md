...
**This prompt is for the `home` context.** Data sources: Gmail, Google Calendar, Todoist, Obsidian.
...
- **Obsidian notes**: `[note title](obsidian://open?vault={vault}&file={filepath_without_extension})` — use the vault name from the config
...
2. **Fetch data sources** (run all four in parallel where possible)
   ...
   **Gmail** — `query_gmail_emails`
   - Account: use the email account from the config
   ...
   **Calendar** — `get_calendar_events`
   - Account: use the calendar account from the config
...
5. **Synthesise the brief** — Produce the following sections:
   ...
   **Carry-overs & Follow-ups**
   ...
   - Recurring themes flagged in step 3, linked to the journal entries where they appeared
