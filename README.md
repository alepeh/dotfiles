...
* **Vault-resident agent skills** (opt-in, requires an Obsidian vault with an `agents/` directory): `make link-vault-skills` symlinks `<vault>/agents/<name>/commands/*.md` into `~/.claude/commands/` so agents defined in the vault (currently the Hudson productivity coach) become available as slash commands in any Claude CLI session on the machine. Vault path defaults to `~/obsidian/brain`; override with `HUDSON_VAULT=/path/to/vault`. Idempotent — re-run any time agent commands are added or renamed. See [`scripts/link-vault-skills.sh`](scripts/link-vault-skills.sh).
